...
def main():
    ...
    # Camera capture #####################################################
    #ret, image = cap.read()
    image = cv.imread( 'output/output.jpg' )
    #if not ret:
    #    break
    ...
    if results.multi_hand_landmarks is not None:
        for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
            ...
            if keypoint_classifier_labels[hand_sign_id] == 'Open' :
                ...
                gestureResult = keypoint_classifier_labels[hand_sign_id]
    else:
        point_history.append([0, 0])

    debug_image = draw_point_history(debug_image, point_history)
    debug_image = cv.flip(debug_image, 1) 
    debug_image = draw_info(debug_image, gestureResult, mode, number)

    # Screen reflection #############################################################

    #cv.imshow('Hand Gesture Recognition', debug_image)
    return gestureResult, debug_image
#    cap.release()
...
def draw_info(image, fps, mode, number):
    cv.putText(image, "Gesture:" + str(fps), (10, 30), cv.FONT_HERSHEY_SIMPLEX,
